...
int ecount=0,icount=0,scount=0,rcount=0;
		...
		for(i=0;i<population;i++)
			for(j=0;j<population;j++)
				if(i!=j)
					...
					{
						if(nodes[i].dist[j]>nodes[i].dist[k]+nodes[k].dist[j] && nodes[k].state!='I' && nodes[k].state!='C')
						{
...
void computeState()
{
	...
	for(i=0;i<population;i++)
	{
		...
		else if(nodes[i].buffAvailable == 0 && nodes[i].state == 'E')
		{
			scount--;
			nodes[i].state='I';
			ecount++;
...
int main(int argc,char* argv[])
{
	...
	for(i=0;i<nsrc;i++)
	{
		...
		nodes[srcCalc].state='C';//source nodes are carriers
